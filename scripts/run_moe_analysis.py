...
def do_token_analysis_layers(args, tex_format=True):
    ...
    ax.set_ylabel("Token specialization (%)", fontsize=FONTSIZE, fontweight='bold')
    ax.spines['top'].set_visible(False)
    ax.spines['right'].set_visible(False)
...
def do_token_analysis_experts(args, tex_format=True, do_sort=False):
    ...
    plt.legend(frameon=True, fontsize=FONTSIZE, columnspacing=0.4, labelspacing=0.4, loc="lower right")
    plt.savefig(os.path.join(args.fig_dir, f"token_specialization_experts.pdf"))
    plt.savefig(os.path.join(args.fig_dir, f"token_specialization_experts.png"))
    

def do_token_analysis_layers_experts(args, do_sort=True, normalize=False):
    ...
    ax.plot(x, y, label="Ground-truth output tokens", color="#2E3168", marker='o', markersize=12, linewidth=6)
    # Draw horizontal prob lines for input_prob etc
    ax.axhline(input_prob_chosen * 100, color="#F0539B", linestyle='--', linewidth=3)
    ax.axhline(pred_prob_chosen * 100, color="#43C5E0", linestyle='--', linewidth=3)
    ax.axhline(gt_prob_chosen * 100, color="#2E3168", linestyle='--', linewidth=3)
    ax.tick_params(axis='both', which='major', labelsize=FONTSIZE)
    #ax.set_xlim(0, x[-1])
    ax.margins(x=0.005)
    ax.set_xticks(x, x_sorted)
    ...
    plt.legend(frameon=True, fontsize=FONTSIZE, columnspacing=0.4, labelspacing=0.4, loc="lower right")
    plt.savefig(os.path.join(args.fig_dir, f"token_specialization_top{args.topk}_{args.model}.pdf"))
    plt.savefig(os.path.join(args.fig_dir, f"token_specialization_top{args.topk}_{args.model}.png"))
...
if __name__ == '__main__':
    ...
    To do correlation analysis (Expert co-activation):
    ...
    To do token analysis (Token specialization):
